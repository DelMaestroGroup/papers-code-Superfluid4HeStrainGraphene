...
#             axs[i].set_ylabel('SF fraction')
    ...
    return fig
# -----------------------------------------------------------------------------
def plot_only_two(result, result2):
    '''using result of esti_array, plot n/totN vs totN for each temperature'''
    '''also returns mean of it and maximum of error'''
    _x = result['totN'][0]
    _x1 = np.asarray([1/(item) for item in _x])
...
#                                     sharex=True, figsize = (3.4646*2, 2.14122*nrow) )
    ...
    #     plt.setp(axs, xlim=custom_xlim2, ylim=custom_ylim2)
        ...
        ax2.set_xlim(custom_xlim1)
        ax2.set_ylim(custom_ylim1)
        # add panel labels
        ...
        for i in range(len(result['Tset'])):
            ...
            # plot the data
            ax1.errorbar(x1, y1, yerr = σ1, color=colors[i], fmt='.', ms = 5, label=f'T={tag} QMC')
            ax2.errorbar(p1, k1, yerr = σb1, color=colors[i], fmt='.', ms = 5, label=f'T={tag} QMC')
            ...
            # plot the fit results
            fx2 = np.linspace(0,max(x2)*1.1,20)
            axs[2*i].plot(fx2,a1s[0]+a1s[1]*fx2, color=colors[i], linewidth=1.5, zorder=0, label=f'T={tag} fit')
            fxb2 = np.linspace(0,max(x2)*1.1,20)
            axs[2*i+1].plot(fxb2,a1bs[0]+a1bs[1]*fxb2, color=colors[i], linewidth=1.5, zorder=0, label=f'T={tag} fit')

            axs[2*i].set_xlim(custom_xlim2)
            axs[2*i].set_ylim(custom_ylim2)
            axs[2*i].legend(loc=2)
            axs[2*i].set_ylabel('Superfluid fraction')
            axs[2*i+1].set_xlim(custom_xlim2)
            axs[2*i+1].set_ylim(custom_ylim2)
            axs[2*i+1].legend(loc=2)
